...
class GroupSpec extends Specification {
  ...
  "Group" should {
    ...
    "find by primary keys" in new AutoRollback {
      val maybeFound = Group.find(123)
      maybeFound.isDefined should beTrue
    }
    "find by where clauses" in new AutoRollback {
      val maybeFound = Group.findBy(sqls.eq(g.id, 123))
      maybeFound.isDefined should beTrue
    }
    ...
    "find all by where clauses" in new AutoRollback {
      val results = Group.findAllBy(sqls.eq(g.id, 123))
      results.size should be_>(0)
    }
    "count by where clauses" in new AutoRollback {
      val count = Group.countBy(sqls.eq(g.id, 123))
      count should be_>(0L)
    }
    "create new record" in new AutoRollback {
      val created = Group.create(idProject = 123, name = "MyString", createdDate = null)
    ...
    "destroy a record" in new AutoRollback {
      val entity = Group.findAll().head
      val deleted = Group.destroy(entity) == 1
      deleted should beTrue
      val shouldBeNone = Group.find(123)
