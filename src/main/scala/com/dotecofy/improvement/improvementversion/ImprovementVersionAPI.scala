...
trait ImprovementVersionAPI extends ScalatraBase with FutureSupport with JacksonJsonSupport with CorsSupport {

  implicit val services: ImprovementVersionServicesComponentSub = ImprovementVersionServicesSub
  ...
  implicit val repVersion: VersionRepositoryComponentSub = VersionRepositorySub
  ...
  post("/") {
    services.create(ParseHeaders.authorization(request).orNull, parsedBody.extract[ImprovementVersionSrv])
  }

  put("/:signature") {
    services.update(ParseHeaders.authorization(request).orNull, params("signature"), parsedBody.extract[ImprovementVersionSrv])
  }
  ...
  post("/fsearch") {
    ...
    val improvementversions = services.findByProfile(ParseHeaders.authorization(request).orNull, (req.pageNumber - 1) * req.pageSize, req.pageSize, (() => { if (req.searchKey == null || req.searchKey.isEmpty) { "name" } else { req.searchKey } }).apply(), req.searchValue).right.get
    FSearchRes(improvementversions.length, improvementversions)
  }
